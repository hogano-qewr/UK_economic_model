...
eq_UIP <- RER_GAP ~ 
  lag(RER_GAP, 1) + 
  lag(R_GAP_diff, 1)
system <- list(IS = eq_IS, PC = eq_PC, TR = eq_TR, Okun = eq_Okun, UIP = eq_UIP)

# 2. Define the Instruments
# These must be EXOGENOUS (lags, dummies, or external shocks)
instr <- ~ Y_GAP_lag1 + INFL_yoy_lag1 + lag(i_GAP,1) + U_GAP_lag1 + 
...
ggplot(MODEL_READY, aes(x = R_GAP, y = Y_GAP)) +
  geom_point(alpha = 0.5) + 
  geom_smooth(method = "lm", color = "red") + 
  labs(title = "The IS Curve: Real Interest Rates vs Output Gap",
       subtitle = "Coefficient: -1.30 (Significant at 5%)",
       x = "Real Interest Rate Gap",
       y = "Output Gap") +
  theme_minimal()

# 1. Get the residuals from your IS equation
is_resids <- residuals(fit_3sls)$IS

# 2. Add back the specific effect of R_GAP to those residuals
# Replace -1.30489815 with your exact R_GAP coefficient if it changed
partial_y <- is_resids + (-1.30489815 * MODEL_READY$R_GAP)

# 3. Plot the "Clean" relationship
library(ggplot2)
ggplot(MODEL_READY, aes(x = R_GAP, y = partial_y)) +
  geom_point(alpha = 0.4, color = "black") + 
  geom_smooth(method = "lm", color = "red", size = 1.2) + 
  labs(title = "Causal IS Curve: The Impact of Real Interest Rates",
       subtitle = "Slope = -1.30 (Controlling for COVID, RER, and Lags)",
       x = "Real Interest Rate Gap",
       y = "Output Gap (Partial Residuals)") +
  theme_minimal()
...
# 2. Calculate Partial Y for Credit Spread (using your -2.0746 coefficient)
partial_y_spr <- is_resids + (-2.07461181 * MODEL_READY$i_spr_GAP)

# 3. Plot
ggplot(MODEL_READY, aes(x = i_spr_GAP, y = partial_y_spr)) +
  geom_point(alpha = 0.4, color = "darkblue") + 
  geom_smooth(method = "lm", color = "darkblue", size = 1.2) + 
  labs(title = "The Credit Channel: Spreads vs Output Gap",
       subtitle = "Slope = -2.07 (Significant at 5%)",
       x = "Interest Rate Spread Gap",
       y = "Output Gap (Partial Residuals)") +
  theme_minimal()


# 1. Get UIP residuals
uip_resids <- residuals(fit_3sls)$UIP

# 2. Calculate Partial Y for Interest Differential (using your -0.4961 coefficient)
# Note: Ensure lag(R_GAP_diff, 1) matches your dataframe column name
partial_y_uip <- uip_resids + (-0.49614136 * MODEL_READY$R_GAP_lag1)

# 3. Plot
ggplot(MODEL_READY, aes(x = lag(R_GAP_lag1, 1), y = partial_y_uip)) +
  geom_point(alpha = 0.4, color = "darkgreen") + 
  geom_smooth(method = "lm", color = "darkgreen", size = 1.2) + 
  labs(title = "UIP Relation: Interest Differentials vs RER",
       subtitle = "Slope = -0.49 (Significant at 1%)",
       x = "Lagged Real Interest Rate Differential",
       y = "Real Exchange Rate Gap (Partial Residuals)") +
  theme_minimal()
...
is_test <- ivreg(Y_GAP ~ 
  ...
  dummy_2020Q2 + dummy_2020Q3 | 
                   ...
                   RER_GAP_lag2 + dNX_lag1 + dNX_lag2 + dlog_IM_defl_lag1 + dlog_energy_cpi_lag1 + 
...
is_test_trimmed <- ivreg(
  Y_GAP ~ 
    ...
    Y_GAP_lag1 + INFL_yoy_lag1 + lag(i_GAP,1) + U_GAP_lag1 + 
    RER_GAP_lag1 + RER_GAP_lag2 + dNX_lag1 + dNX_lag2 + 
    dlog_IM_defl_lag1 + dlog_energy_cpi_lag1 + dummy_2020Q2 + dummy_2020Q3, 
  data = MODEL_READY
)
...
system_final <- list(IS = eq_IS_final, PC = eq_PC, TR = eq_TR, Okun = eq_Okun, UIP = eq_UIP)
...
#### ABOVE MODELS ARE STATISTICALLY STRONG BUT ECONOMICALLY WEAK #####################

### TRY CONSTRAINING COEFFICIENTS ON LEADS / LAGS (SO THEM SUM TO 1)

# Define the restriction: IS_Y_GAP_lag1 + IS_Y_GAP_lead1 = 1
# (The exact name depends on your equation labels in systemfit)
restriction <- "IS_Y_GAP_lag1 + IS_Y_GAP_lead1 = 1"

# Re-run the model with the restriction
fit_restricted <- systemfit(system, 
                            method = "3SLS", 
                            inst = instr, 
                            data = MODEL_READY,
...
# Testing if Monetary Policy variables actually matter as a group
linearHypothesis(fit_3sls, c("IS_R_GAP = 0", "IS_i_spr_GAP = 0", "UIP_lag(R_GAP_diff, 1) = 0"))
...
# 2. Run the Diagnostic (Sargan is the 'Overidentifying restrictions' part)
summary(IS_test1, diagnostics = TRUE)



# Load the necessary library if not already active
library(systemfit)

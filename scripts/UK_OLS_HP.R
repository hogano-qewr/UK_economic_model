...
IS_fit <- lm(
  Y_GAP ~ 
    ...
    R_GAP + d_mkt_spr + zlb_interaction + 
...
PC_fit <- lm(
  INFL_yoy ~ 
    ...
    dIM_defl_lag1 +   # Imported Inflation (OBR focus)
    dEnergy_lag1 +    # Energy shock (OBR focus)
...
PC_fit_energy <- lm(
  INFL_yoy ~ 
    ...
    dIM_defl_lag1 + 
    dEnergy_lag1 + 
    dlog_price_cap_lag1 + # Regulatory Pass-through
    dummy_2020Q2 + dummy_2020Q3, 
  data = MODEL_READY
)
...
TR_fit <- lm(
  BOE_rate ~ 
    BOE_rate_lag1 +   # Interest rate smoothing
    Infl_dev +        # Reaction to inflation gap
    Y_GAP +           # Reaction to output gap
    zlb_dummy +       # Control for the 0.5% floor period
